...
def load_pemu_table():
    ...
    kpi_pemu[['AVG OAP', 'MAA Pemukiman']] = kpi_pemu[['AVG OAP', 'MAA Pemukiman']].astype(int)
    return kpi_pemu                      
...
with colojol1:
    ...
    with tab2:
        ...
        fig = px.line(ojol_wely, height = 280, width= 425).update_layout(yaxis_ticksuffix = 1, showlegend = False).update_xaxes(dtick = 1)
        st.plotly_chart(fig)
